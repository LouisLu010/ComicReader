...
| M2.3 | 完整 iPad 体验与隐私保护 | 功能已接入；自动化结果以 CI 为准，真机与外接显示器验收待完成 |
...
书库工具栏可以新建窗口，漫画详情可将同一本或不同漫画打开到独立窗口；每个阅读窗口拥有独立导航和阅读会话。使用系统 Split View／Stage Manager 调整窗口大小，支持在兼容 iPad 上将窗口移动到外接显示器。窗口路由支持系统恢复，原图仍只保存在 App 本地书库。

“设置 → 应用锁”可开启 Face ID／Touch ID 或设备密码认证；开启、关闭均需认证。窗口失活时立即以不含漫画内容的遮罩保护预览，包括已打开的弹窗。App 进入后台或冷启动后锁定，取消／失败不会解锁。App 不存储认证密码或生物识别信息。

“阅读显示”可从设置页或阅读器工具栏打开，提供背景、应用内亮度、纯色留白裁切、90° 步进旋转、阅读常亮和动画开关。阅读器中的“仅当前页”可独立设置该页旋转并持久保存，也可恢复默认旋转，不影响其他页面或漫画。亮度只调整阅读内容，不修改系统亮度；裁切和旋转仅影响展示图。系统“降低动态效果”优先于动画开关，高对比度下控件使用实色背景，大字号下详情纵向排列并省略缩略图带，页码和翻页操作仍可用。

| 快捷键 | 操作 |
|---|---|
| ⌘N / ⌘O | 新建窗口／导入文件夹 |
| ⌘F / ⌘, | 书库搜索／设置 |
| 左右方向键 / 空格 | 翻页（方向键遵循阅读方向）／下一页 |
| ⌥ 左右方向键 / ⌘L | 上下话／章节列表 |
| ⌘+ / ⌘− / ⌘H | 放大／缩小／显示隐藏控件 |
| ⇧⌘D / ⌃⌘L | 阅读显示／立即锁定 |

硬件和辅助功能验收范围见 [M2.3 验收记录](docs/M2_3_ACCEPTANCE.md)。模拟器通过不等于真机 Face ID 或外接显示器已通过。
